    ...
    def post(self, request):
        ...
        try:
            ...
            obj = UserFile.objects.create(
                ...
                is_valid=True,  # for now assume valid until text check in Agent 2
                validation_reason="Passed initial validation"
            )

            return JSONResponseSender.send_success(
                {"file_id": obj.id, "status": "uploaded", "valid": obj.is_valid}
            )
...
class PreprocessFileView(APIView):
    def post(self, request, file_id):
        try:
            preprocess_file_task(file_id)
            # preprocess_file_task.delay(file_id)
            return JSONResponseSender.send_success({"status": "queued", "file_id": file_id})
        except Exception as e:
            return JSONResponseSender.send_error("500", str(e), str(e))
